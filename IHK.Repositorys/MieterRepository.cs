...
namespace IHK.Repositorys
{
    public class MieterRepository
    {
        ...
        public async Task<Wohnung> GetWohnungById(int id)
        {
            return await _dataContext.Wohnung.Include(w => w.Gebaeude).ThenInclude(g => g.Adresse).SingleOrDefaultAsync(w => w.Id == id);
        }
        ...
        {
            _dataContext.Mieters.Add(mieter);
        }
